...
func TestTokens(t *testing.T) {
	...
	// Create test users.
	user1, err := data.ReadOrCreateUser(userName, email)
	assert.NoError(t, err)
	user2, err := data.ReadOrCreateUser(userName2, email2)
	assert.NoError(t, err)

	// Test can get user by token.
	err = data.WriteToken("a", user1.Name, user1.Email, "", "")
	assert.NoError(t, err)
	fetchedUser, err := data.UserByToken("a")
	assert.NoError(t, err)
	assert.Equal(t, user1.ID, fetchedUser.ID)
	assert.Equal(t, "a", fetchedUser.Token)

	// Test can get user by token for a different user and token.
	err = data.WriteToken("b", user2.Name, user2.Email, "", "")
	assert.NoError(t, err)
	fetchedUser, err = data.UserByToken("b")
	assert.NoError(t, err)
	assert.Equal(t, user2.ID, fetchedUser.ID)
	assert.Equal(t, "b", fetchedUser.Token)

	// Test can't get user after revoking token
	err = data.RevokeToken("a", user1.Email)
	assert.NoError(t, err)
	fetchedUser, err = data.UserByToken("a")
	assert.Error(t, err)
	assert.Nil(t, fetchedUser)

	// Test can't revoke token with wrong user email.
	err = data.RevokeToken("b", user1.Email)
	assert.Error(t, err)

	// Test token still exists after failed revoke.
	fetchedUser, err = data.UserByToken("b")
	assert.NoError(t, err)
	assert.Equal(t, user2.ID, fetchedUser.ID)
	assert.Equal(t, "b", fetchedUser.Token)

	// Test can revoke properly after failed revoke.
	err = data.RevokeToken("b", user2.Email)
	assert.NoError(t, err)
	fetchedUser, err = data.UserByToken("b")
	assert.Error(t, err)
	assert.Nil(t, fetchedUser)
	...
	err = data.WriteToken("a", user1.Name, user1.Email, "", "")
	assert.NoError(t, err)
	err = data.WriteToken("b", user1.Name, user1.Email, "", "")
	assert.NoError(t, err)
	err = data.WriteToken("c", user1.Name, user1.Email, "", "")
	assert.NoError(t, err)

	// Test gets correct token.
	fetchedUser, err = data.UserByToken("a")
	assert.NoError(t, err)
	assert.Equal(t, user1.ID, fetchedUser.ID)
	assert.Equal(t, "a", fetchedUser.Token)
	...
	assert.Equal(t, user1.ID, fetchedUser.ID)
	assert.Equal(t, "b", fetchedUser.Token)
	fetchedUser, err = data.UserByToken("c")
	assert.NoError(t, err)
	assert.Equal(t, user1.ID, fetchedUser.ID)
	assert.Equal(t, "c", fetchedUser.Token)

	// Test revokes correct token.
	err = data.RevokeToken("c", user1.Email)
	assert.NoError(t, err)
	err = data.RevokeToken("c", user1.Email)
	assert.Error(t, err)
	fetchedUser, err = data.UserByToken("a")
